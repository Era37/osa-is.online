...
pub async fn cache_data<'a, T>(key: String, filter: impl Into<Option<Document>>, database: &'a Db, collection: &str) -> Option<Vec<T>> where T: DeserializeOwned + Serialize + std::fmt::Debug {
    ...
    if value != None {
        ...
        let collection: Collection<Document> = database.mongo.collection(collection);
